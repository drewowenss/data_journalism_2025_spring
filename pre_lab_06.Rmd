...
```{r}
voters_by_county <- read_csv("New Active Voting Data.csv")
...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** There are no haeder rows, some data repeats itself, we really do not know what we are looking at, and there are some rows that show REP in Congress or other weird names and have zero data, which we cannot have.
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Having X2 and X3 and so on headers is not clear. Also we should divide by party and make it their own column and what office they are in. 
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** We have actual headers and separated the data and what party the people are in. On the second page, there are still missing N/A data in some rows and columns. 
