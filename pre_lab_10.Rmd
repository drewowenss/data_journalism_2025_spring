...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** COLUMNS THAT I SEE ARE PRECINCT ID AND NAME, CONGRESS, LEGIS, COUNCIL, SCHOOL, POLLING TD, AND SHAPE LENGTH, AREA AND SHAPE. A LOT OF THIS DATA IS CHARACTERISTICS RATHER THAN DBL, WHEN SOME ARE NUMBERS WHICH IS WEIRD.
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** THIS INFORMATION SHOWS PG COUNTY ELEMENTARY PUBLIC SCHOOLS, THEIR PRECINCT ID, SHAPES, AND COUNCIL MEMBERS/CONGRESS.
...
**Task** Run the following code. Describe the output in the space below. **Answer** ALL OF THE ZIPCODES THAT ARE PRESENT WITHIN PG COUNTY.
...
**Task** Run the following code. Describe the output in the space below. **Answer** I DO NOT SEE ANYTHING.
...
**Task** Run the following code. Describe the output in the space below. **Answer** THIS IS VERY CONFUSING. I SEE DIFFERENT TOPICS SUCH AS STATEFP, COUNTYFP, FUNCSTATE, AND INTPTLON. I ALSO SEE A MULTIPOLYGON. AND YOU ALSO SEE HIGH SCHOOLS, AND MIDDLE SCHOOLS. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** THIS IS MORE SO BASED ON THE COUNTIES, AND YOU SEE ALL THE COUNTIES, WITH ALL THESE NUMBERS.
...
**Task** Run the following code. Describe the output in the space below. **Answer** ALL I SEE IS A MAP OF MARYLAND AND ITS 24 COUNTIES.
...
**Task** Run the following code. Describe the output in the space below. **Answer** I SEE THE ZIP CODES OF PG COUNTY OR THEY ARE JUST CITIES OF PG COUNTY SINCE ZIPCODES CAN OVERLAP I THINK. THEY COULD ALSO BE BOUNDARIES OF SCHOOL ZONES.
...
**Task** Run the following code. Describe the output in the space below. **Answer** THE OUTPUT SHOWS TOTALS IN POLLING, ABSENTEE, ELIBIGLE AND EARLY IN REGARDS TO PRECINCT.
...
**Task** Run the following code. Describe the output in the space below. **Answer** NOW YOU SEE THE NAME, THE VOTING CENTERS, HOW EACH CENTERED POLLED OR IF THEY HAD A CERTAIN AMOUNT OF VOTERS (ABSENTEE, ELIGIBLE, OR OTHER). THIS IS VOTER DATA ESSENTIALLY.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU SEE COLOR WITHIN THE MAP ON TOTAL ELIGIBLE, FROM     WHITE TO BLUE THROUGHOUT THE BOUNDARIES OF PG COUNTY.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU HAVE COUNTIES, AND HOW MANY PEOPLE ARE A CERTAIN POLITICAL AFFILIATION. AND THE TOTAL AMOUNT IN EACH COUNTY THAT ARE ALSO REGISTERED TO VOTE.
...
**Task** Run the following code. Describe the output in the space below. **Answer** API KEY IS NOT RUNNING SO I HAD TO ADD A NEW ONE. I SEE A GEOID, A VARIABLE THAT IS NAMED POPULATION OR DISPLAYS POPULATION, ESTIMATE AND MOE.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU PRACTICALLY HAVE MARYLAND POPULATION BY COUNTY AND THE POPULATION, THE VARIABLE ESTIMATE AND MOE, BUT THIS IS IN REGULAR TABLE FORM, THEY WANT IT INTO A MAP ESSENTIALLY. AND YOU HAVE A NEW COLUMN, MULTIPOLYGON.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU HAVE THE COUNTIES WITH THE AFFILATION THE PEOPLE ARE BY COUNT AND THEN THE TOTAL NUMBER ACROSS THE COUNTIES. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** HAVE THE COLUMNS WITH THE COUNTIES, THE POPULATION OF THE COUNTIES, THE MULTIPOLYGON, AND GEOMETRY.
...
**Task** Run the following code. Describe the output in the space below. **Answer** WE ESSENTIALLY HAVE THE SAME THING, HOWEVER WE HAVE N/A IN SOME OF THE DATA SETS.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU HAVE THE COUNTIES WITH THE POPULATION AND THEIR GEOMETRY.
...
**Task** Run the following code. Describe the output in the space below. **Answer** THIS IS A TABLE WITH COUNTIES, THEIR POPULATIONS, THEIR AFFLIATIONS OF PEOPLE, THE TOTAL VOTERS, AND THE MULTIPOLYGON.
...
**Task** Run the following code. Describe the output in the space below. **Answer** I SEE A CHART WITH VOTERS PER 10K WITH THE MOST BEING IN COUNTIES SUCH AS PG, MOCO, AND ALL THE WAY DOWN ON THE EASTERN SHORE.
...
**Task** Run the following code. Describe the output in the space below. **Answer** DIFFERENT COLORS OF MOST TO LEAST IN THE COUNTIES AND NOW ITS ALL COLOR CODED.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** SEEMS TO BE A LOT OF THE COUNTIES WITH MORE SO POPULATED VOTERS ARE DARKER COLORS AND VICE VERSA.
...
**Task** Run the following code. Describe the output in the space below. **Answer** YOU BASICALLY DID THE SAME THING BUT ALSO ADDED UNA VOTERS.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** PG AND BMORE COUNTY MAKE SENSE - BECAUSE THEY ARE HEAVILY POPULATED. HOWEVER, THE LIGTHER ONES ARE IN YELLOW. 
