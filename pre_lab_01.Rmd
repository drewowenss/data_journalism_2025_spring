...
```{r}
#131395 observations of 24 variables.
```
...
```{r}
#last_name is <chr> and amount is <dbl>. I feel as though last name represents a characteristic and amount is data in decimals.
```
...
**Answer**
```{r}
#I think that this code is collecting data for county contributions for each county and how many people contributed on a certain date. And this code is organized by the date. The answer represents how many people contributed to a certain person or business from each county in Maryland is what I am assuming. Except they took out the names and all of the personal data. 
```
...
**Answer**
```{r}
#The dates have been all switched around. The previous chart, the dates were in date order, and on this one, they are all mixed up. This affects the results because then everything is kind of jumbled. Say you have a data plot that you are trying to make, your data is going to be scattered, and you cannot compare by month and day. 
```
...
**Answer**
```{r}
#There is no date anymore, instead the columns are now memo-text and amount that are new. This affects the result because we do not know when or who contributed to where ever they donated.
```
...
**Answer**
```{r}
#Literally everything is gone from this data but the memo_text. This affects the result because we only see the memo_text, we do not know who gave money, what county gave money, when they gave the money, and all the important details that would be needed in a data set. 
```
...
**Answer**
```{r}
#This is different from the following code block because it adds the the mean and median amount of people who contribute. 
```
...
```{r}
#This block shows the maximum and minimum amounts of contributions, along with the cities unlike the last block above. 
```
